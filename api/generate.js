...
const responseSchema = {
    ...
    items: {
        ...
        properties: {
            ...
            "recipe": { 
                type: "STRING",
                description: "5ステップ程度の簡潔な調理手順を必ず番号付きリスト（例: 1. ..., 2. ...）で、必ず１行ずつ箇条書き、目安調理時間も表示、最後にトータル調理時間を表示するMarkdown形式で記述。"
            },
        },
...
function buildPrompt(ingredients, mode, allergens) {
    ...
    switch (mode) {
        ...
        default:
            // 通常モード
            systemInstruction += "【制約】一般的な家庭で作りやすい、バラエティに富んだ子どもも喜ぶ献立を提案してください。";
            break;
    }
    ...
    systemInstruction += `【重要】全ての献立を提案するにあたり、現在の物価高に加え、国民の手取り額も上がらない状況を考慮し、できるだけ安価な食材（もやし等）・調味料を使って現実的なものにすること。`;
